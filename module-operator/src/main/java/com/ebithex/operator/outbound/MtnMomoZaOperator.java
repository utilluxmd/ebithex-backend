...
/**
 * Adaptateur MTN Mobile Money — Afrique du Sud (Collection + Disbursement)
 *
 * API : MTN MoMo Developer Platform (sandbox.momodeveloper.mtn.com)
 * Indicatif : +27 | Monnaie : ZAR
 */
@Component
@Slf4j
public class MtnMomoZaOperator implements MobileMoneyOperator {

    private static final String COLLECTION_TOKEN_KEY   = "mtn-momo-za-collection-token";
    private static final String DISBURSEMENT_TOKEN_KEY = "mtn-momo-za-disbursement-token";
    ...
    @Value("${ebithex.operators.mtn-za.disbursement-api-user:${ebithex.operators.mtn-za.api-user:}}")
    private String disbursementApiUser;

    @Value("${ebithex.operators.mtn-za.disbursement-api-key:${ebithex.operators.mtn-za.api-key:}}")
    private String disbursementApiKey;

    @Value("${ebithex.operators.mtn-za.disbursement-subscription-key:${ebithex.operators.mtn-za.subscription-key:}}")
    private String disbursementSubscriptionKey;

    @Value("${ebithex.operators.mtn-za.disbursement-callback-url:}")
    private String disbursementCallbackUrl;

    public MtnMomoZaOperator(
            WebClient.Builder webClientBuilder,
            OperatorTokenCache tokenCache,
            @Value("${ebithex.operators.mtn-za.base-url:https://sandbox.momodeveloper.mtn.com}") String baseUrl,
            @Value("${ebithex.operators.mtn-za.subscription-key:}") String subscriptionKey,
            @Value("${ebithex.operators.mtn-za.api-user:}") String apiUser,
            @Value("${ebithex.operators.mtn-za.api-key:}") String apiKey,
            @Value("${ebithex.operators.mtn-za.environment:sandbox}") String environment,
            @Value("${ebithex.operators.mtn-za.callback-url:}") String callbackUrl) {
    ...
    public OperatorType getOperatorType() {
        return OperatorType.MTN_MOMO_ZA;
    }
    ...
    public OperatorInitResponse initiatePayment(OperatorPaymentRequest request) {
        ...
        try {
            ...
            log.info("MTN ZA requesttopay accepted — ebithexRef={} mtnRef={}", request.ebithexReference(), referenceId);
            return OperatorInitResponse.processing(referenceId, null,
                "Approve the payment on your MTN MoMo South Africa app");

        } catch (WebClientResponseException e) {
            log.error("MTN ZA collection error {} — {}", e.getStatusCode(), e.getResponseBodyAsString());
            if (e.getStatusCode() == HttpStatus.UNAUTHORIZED) tokenCache.invalidate(COLLECTION_TOKEN_KEY);
            return OperatorInitResponse.failed("MTN ZA rejected payment: " + e.getStatusCode());
        }
    }
    ...
    public OperatorInitResponse initiateDisbursement(OperatorDisbursementRequest request) {
        ...
        try {
            ...
            log.info("MTN ZA transfer accepted — ebithexRef={} mtnRef={}", request.ebithexReference(), referenceId);
            return OperatorInitResponse.processing(referenceId, null,
                "Disbursement in progress — beneficiary will receive MTN ZA notification");

        } catch (WebClientResponseException e) {
            log.error("MTN ZA disbursement error {} — {}", e.getStatusCode(), e.getResponseBodyAsString());
            if (e.getStatusCode() == HttpStatus.UNAUTHORIZED) tokenCache.invalidate(DISBURSEMENT_TOKEN_KEY);
            return OperatorInitResponse.failed("MTN ZA rejected disbursement: " + e.getStatusCode());
        }
    }
    ...
    private String getOrRefreshToken(String cacheKey, String user, String key, String tokenUri) {
        ...
        if (resp == null) throw new IllegalStateException("MTN ZA: empty token response for " + tokenUri);
